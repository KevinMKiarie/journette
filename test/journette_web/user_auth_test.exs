defmodule JournetteWeb.UserAuthTest do
  use JournetteWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Journette.Accounts
  alias Journette.Accounts.Scope
  alias JournetteWeb.UserAuth

  import Journette.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, JournetteWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      JournetteWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        JournetteWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
